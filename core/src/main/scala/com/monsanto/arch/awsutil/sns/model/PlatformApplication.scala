...
import com.monsanto.arch.awsutil.sns.model.PlatformApplication._
...
/** Represents a snapshot of the state of a platform application. Note that this is a immutable object and is not live,
  ...
  */
case class PlatformApplication private[sns] (arn: String,
  ...
  def name: String = arn match { case ArnRegex(_, n) ⇒  n }
  /** Returns the push notification platform for this application. */
  def platform: Platform = arn match { case ArnRegex(Platform(p), _) ⇒ p }
...
object PlatformApplication {
  ...
  /** Regular expression to extract the components of a platform application ARN. */
  private val ArnRegex = "^arn:aws:sns:[^:]+:[^:]+:app/([^/:]+)/([^/:]+)$".r
}
