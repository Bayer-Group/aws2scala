...
import com.monsanto.arch.awsutil.sns.model.PlatformEndpoint.ArnRegex
...
/** Represents a snapshot of the state of a platform endpoint.  Note that this is a immutable object and is not live,
  ...
  */
case class PlatformEndpoint private[sns] (arn: String, attributes: Map[String,String]) {
  ...
  def platform: Platform = arn match { case ArnRegex(Platform(p)) ⇒ p}
  ...
  /** Sends a JSON message to this platform endpoint.  If you would rather send a plain string, use `publish` instead.
    ...
    * @return the unique identifier of the published message
    */
  def publishJson(jsonMessage: String, attributes: Map[String,MessageAttributeValue[_]])
                 (implicit m: Materializer, sns: StreamingSNSClient): Future[String] =
...
object PlatformEndpoint {
  ...
  /** Builds a `PlatformEndpoint` object from its AWS equivalent. */
  def apply(aws: Endpoint): PlatformEndpoint =
    PlatformEndpoint(aws.getEndpointArn, aws.getAttributes.asScala.toMap)

  /** Regular expression to extract the components of a platform endpoint ARN. */
  private val ArnRegex = "^arn:aws:sns:[^:]+:[^:]+:endpoint/([^/:]+)/[^/:]+/[^/:]+$".r
}
