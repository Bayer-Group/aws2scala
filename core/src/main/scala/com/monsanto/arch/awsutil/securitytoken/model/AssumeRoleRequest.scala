...
case class AssumeRoleRequest(roleArn: String,
  ...
  def toAws: AwsAssumeRoleRequest = {
    val request = new AwsAssumeRoleRequest()
      .withRoleArn(roleArn)
      .withRoleSessionName(roleSessionName)
    duration.foreach(d ⇒ request.setDurationSeconds(d.toSeconds.toInt))
    externalId.foreach(id ⇒ request.setExternalId(id))
    policy.foreach(p ⇒ request.setPolicy(p))
    mfa.foreach { mfa ⇒
      request.setSerialNumber(mfa.serialNumber)
      request.setTokenCode(mfa.tokenCode)
    }
    request
  }
}
