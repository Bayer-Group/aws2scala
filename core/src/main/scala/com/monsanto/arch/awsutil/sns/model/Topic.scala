...
import com.monsanto.arch.awsutil.sns.model.Topic._
...
/** Represents the snapshot of the state of a topic.  Note that this is a immutable object and is not live, i.e.
  ...
  */
case class Topic private[sns] (attributes: Map[String,String]) {
  /** The topic’s ARN. */
  ...
  def name: String = TopicArn.unapplySeq(arn).get.head
  ...
  def subscribe(subscriptionEndpoint: SubscriptionEndpoint)
    ...
    Source.single(SubscribeRequest(arn, subscriptionEndpoint.protocol.toAws, subscriptionEndpoint.endpoint))
      .via(sns.subscriber)
  ...
  /** Sends a message to all of the topic’s subscribed endpoints.
    ...
    * @return the unique identifier of the published message
    */
  def publish(message: String, attributes: Map[String,MessageAttributeValue[_]])
             (implicit m: Materializer, sns: StreamingSNSClient): Future[String] =
  ...
  /** Sends a message to all of the topic’s subscribed endpoints.
    ...
    * @return the unique identifier of the published message
    */
  def publish(message: String, subject: String, attributes: Map[String,MessageAttributeValue[_]])
             (implicit m: Materializer, sns: StreamingSNSClient): Future[String] =
  ...
  /** Sends a message to all of the topic’s subscribed endpoints.  This variation allows sending different messages
    ...
    * @return the unique identifier of the published message
    */
  def publish(message: Map[String,String], attributes: Map[String,MessageAttributeValue[_]])
             (implicit m: Materializer, sns: StreamingSNSClient): Future[String] =
    Source.single(PublishRequest(arn, message, attributes))
      .via(sns.publisher)
      .runWith(Sink.head)
  ...
  /** Sends a message to all of the topic’s subscribed endpoints.  This variation allows sending different messages
    ...
    * @return the unique identifier of the published message
    */
  def publish(message: Map[String,String], subject: String, attributes: Map[String,MessageAttributeValue[_]])
             (implicit m: Materializer, sns: StreamingSNSClient): Future[String] =
    Source.single(PublishRequest(arn, message, subject, attributes))
      .via(sns.publisher)
      .runWith(Sink.head)
...
object Topic {
  /** Regular expression used to extract the topic name. */
  private val TopicArn = s"^arn:aws:sns:[^:]+:[^:]+:(.+)$$".r
  ...
  def apply(topicArn: String)(implicit sns: StreamingSNSClient, m: Materializer): Future[Topic] = {
    require(topicArn.matches(TopicArn.regex), s"‘$topicArn’ does not appear to be a valid topic ARN")
    Source.single(topicArn).runWith(toTopic)
  }
}
