...
class SecurityTokenServiceIntegrationSpec extends FreeSpec with AwsIntegrationSpec with StrictLogging with IntegrationCleanup {
  private val iam = streamingAwsClient.identityManagement
  private val async = asyncAwsClient.securityTokenService
  ...
  "the Security Token Service client" - {
    ...
    "can use the credentials" - {
      "to not list SNS topics" in {
        val sns = asyncAwsClient.withCredentialsProvider(credentials).sns
      ...
      "to list buckets" in {
        val s3 = asyncAwsClient.withCredentialsProvider(credentials).s3
